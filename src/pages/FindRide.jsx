...
export default function FindRide() {
  ...
  const [recentPlaces, setRecentPlaces] = useState(() => {
    const saved = localStorage.getItem('sabay_find_recents');
    return saved ? JSON.parse(saved) : [];
  });
  const [savedPlaces, setSavedPlaces] = useState(() => {
    const saved = localStorage.getItem('sabay_find_saved');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const saveRecentPlace = (title, desc, type, lat, lon) => {
    ...
    setRecentPlaces((prev) => {
      ...
      localStorage.setItem('sabay_find_recents', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleSelect = (title, desc = '', lat, lon) => {
    ...
    if (activeField === 'from') {
      ...
      if (isSavedObj && isSavedObj.note) {
         setNoteToDriver(isSavedObj.note);
      } else {
         setNoteToDriver('');
      }
  ...
  const handleFindRideSubmit = async () => {
    ...
    if (!finalFromCoords || !finalToCoords) {
      setErrorMsg("Please specifically select Map-resolved locations securely from the search suggestion dropdown or minimizing map before attempting to find a ride!");
